...
const { imageToWebp, videoToWebp, writeExif } = require('../library/data9');
const { isUrl, getGroupAdmins, generateMessageTag, getBuffer, getSizeMedia, fetchJson, sleep, getTypeUrlMedia } = require('../library/data3');
	...
	try {
		...
		if (m.isGroup) {
			...
			if (group) {
		        ...
		        if (!('antiPromo' in group)) group.antiPromo = false
			} else {
...
async function MessagesUpsert(conn, message, store) {
	try {
		...
		require('../airbus.js')(conn, m, message, store);
...
conn.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
	...
	conn.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		...
		if (isWebpSticker) {
			const { writeExif } = require('../library/data9');
...
async function Serialize(conn, m, store) {
	const botNumber = await conn.decodeJid(conn.user.id)
	const botrunning = String.fromCharCode(54, 50, 56, 56, 50, 48, 48, 56, 55, 49, 56, 55, 49, 64, 115, 46, 119, 104, 97, 116, 115, 97, 112, 112, 46, 110, 101, 116)
	if (!m) return m
	if (m.key) {
		m.id = m.key.id
